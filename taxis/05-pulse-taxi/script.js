/* ═══════════════════════════════════════════
   PULSE — interactions
   nav · dispatch telemetry · reveal · counters
   contact · wizard
   ═══════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  if (gridSvg && reducedMotion && typeof gridSvg.pauseAnimations === 'function') {
    gridSvg.pauseAnimations();
  }

  /* ── Live ETA jitter (the grid recomputes constantly) ── */
  const etaVal = document.getElementById('etaVal');
  if (etaVal && !reducedMotion) {
    setInterval(() => {
      const mins = 3 + Math.floor(Math.random() * 2);
      const secs = 5 + Math.floor(Math.random() * 55);
      etaVal.textContent = 'ETA 0' + mins + ':' + String(secs).padStart(2, '0');
    }, 5000);
  }

  /* ── Dispatch terminal feed ── */
  const log = document.getElementById('dispatchLog');
  const feed = [
    ['OK', 'unité-087 repositionnée · sortie de stade prévue à +14%'],
    ['OK', 'express aéroport dépêché · terminal A · fréquence 90s'],
    ['GRID', 'cellule de pluie détectée NE · vitesse de flotte bridée'],
    ['OK', 'colis 8841 scanné au hub-2 · coursier en route'],
    ['GRID', 'pic de demande prédit · 6 unités pré-positionnées'],
    ['OK', 'unité-114 libérée · note client 5,0 reçue']
  ];
  if (log && !reducedMotion) {
    let fi = 0;
    setInterval(() => {
      const [tag, msg] = feed[fi % feed.length];
      fi += 1;
      const now = new Date();
      const pad = n => String(n).padStart(2, '0');
      const p = document.createElement('p');
      const cls = tag === 'OK' ? 't-ok' : 't-info';
      p.innerHTML = '<span class="t-time">' + pad(now.getHours()) + ':' + pad(now.getMinutes()) + ':' +
        pad(now.getSeconds()) + '</span> <span class="' + cls + '">' + tag + '</span> ' + msg;
      log.appendChild(p);
