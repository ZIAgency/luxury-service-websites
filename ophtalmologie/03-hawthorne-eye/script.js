/* ═══════════════════════════════════════════
   HAWTHORNE EYE CARE — est. 1968
   nav · reveal · counters · ledger · forms
   ═══════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  /* ── The ledger stays current — years in practice, always true ── */
  const YEARS_FOUNDED = 1968;
  const yearsOpen = Math.max(0, new Date().getFullYear() - YEARS_FOUNDED);
  const yearsTargets = ['yearsInPractice', 'badgeYears'].map(id => document.getElementById(id)).filter(Boolean);
  yearsTargets.forEach(el => { el.textContent = String(yearsOpen); });
