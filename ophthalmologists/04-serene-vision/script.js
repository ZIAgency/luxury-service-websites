/* ═══════════════════════════════════════════
   SERENE VISION — eye care studio
   nav · reveal · counters · calm · forms
   ═══════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  /* ── A calm drift — the breathing rings sway with the scroll ── */
  const stage = document.querySelector('.breath-stage');
  if (stage && !reduceMotion) {
    let latest = 0;
    window.addEventListener('scroll', () => { latest = window.scrollY; }, { passive: true });
    (function drift() {
      const sway = Math.sin(latest / 900) * 10;
      stage.style.transform = `translate(calc(-50% + ${sway}px), calc(-50% + ${latest / 90}px))`;
  ...
  function validatePane(pane) {
    ...
    pane.querySelectorAll('[required]').forEach(f => {
      ...
      if (f.type === 'radio') {
        ...
        pane.querySelector('.flow-grid')?.classList.toggle('error', bad);
