/* ═══════════════════════════════════════════
   MERIDIAN PLUMBING CO. — interactions
   nav · reveal · counters · contact · wizard + job sheet
   ═══════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  function fmtDate(v) {
    return v ? new Date(v + 'T12:00:00').toLocaleDateString('fr-FR',
  ...
  document.querySelectorAll('#timeGrid .win').forEach(w => {
    w.addEventListener('click', () => {
      ...
      sum.time.textContent = w.dataset.v;
    });
  });

  /* live job-sheet updates */
  document.querySelectorAll('input[name="svc"]').forEach(r =>
    r.addEventListener('change', () => { sum.svc.textContent = r.value; })
  );
  dateInput.addEventListener('change', () => { sum.date.textContent = fmtDate(dateInput.value); });
  document.getElementById('bk-address').addEventListener('input', (e) => {
    sum.site.textContent = e.target.value || '—';
  });
  ...
  form.addEventListener('submit', (e) => {
    ...
    const when = fmtDate(dateInput.value) + ' · ' + document.getElementById('bk-time').value;
    ...
    document.getElementById('recap-ref').textContent =
      'MPC-' + Math.floor(1000 + Math.random() * 9000);
    cur = 3;
    show(3);
  });
