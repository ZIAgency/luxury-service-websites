/* ═══════════════════════════════════════════
   KEYSTONE LOCK CO. — interactions
   nav · reveal · counters · schematic · contact · wizard
   ═══════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  /* ── Pin-tumbler schematic: insert key, then cycle highlight ── */
  var panel = document.getElementById('tumblerPanel');
  if (panel) {
    var panelObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        if (e.isIntersecting) {
          panel.classList.add('cut');
          panelObserver.unobserve(panel);
          if (!reducedMotion) startPinCycle();
        }
      });
    }, { threshold: 0.35 });
  ...
  function startPinCycle() {
    var cols = panel.querySelectorAll('.pin-col');
    var i = 0;
    setInterval(function () {
      cols.forEach(function (c) { c.classList.remove('hot'); });
      cols[i % cols.length].classList.add('hot');
      i += 1;
    }, 900);
  }

  /* ── Local time readout in panel head ── */
  var clock = document.getElementById('panelClock');
  function tickClock() {
    var d = new Date();
